...
| **Studio v1** | Mini Studio — procedural **Texture Lab**: hand-written seamless noise (value/Perlin/fBm), `.hrt` export + re-editable recipes, in-session collection (`--lab texture`) | ✅ done |
| **Sandbox v1** | Declarative 2D **sandbox** — drag-drop actors, data-only behaviors (mover/spinner/bouncer/lifetime/spawner) + an `OnOverlap` event→action rule on the generic ECS, deterministic `tick`, Play/Stop via scene snapshot, F5/F9 save/load (`--lab scene`) | ✅ done |
...
| **Map / Level Lab v1** | Mini Studio — tile-grid **level editor** (`--lab map`): paint/flood-fill on the shared `fps::Map`, `fpsmap1` text format, palette + save/load collection; `--fps` loads the authored level (`maps/level_00.map`) with a default fallback | ✅ done |
...
| **Particle System v1** | Engine depth (Track A) — reusable **CPU particle sim** (`particles_core`): deterministic seeded xorshift, fractional-accumulator emission, gravity, swap-pop reap, bounded pool, fade helpers; interactive `--lab fx` playground (fountain + click-bursts + live sliders) | ✅ done |
...
| **Tween & Easing v1** | Engine depth (Track A) — the animation interpolation primitive (`tween_core`): 13 named easing curves (`ease()`, clamped, endpoint-pinned; Back/Elastic overshoot) + a deterministic scalar `Tween` (one-shot + no-drift ping-pong); `--lab fx` "sweep" drives the emitter along a `SineInOut` curve | ✅ done |
...
| **Sprite Animation v1** | Engine depth (Track A) — `anim::Flipbook` frame-index player (in `tween_core`): steps a sheet at `fps`, loops (bounded `t`, no drift) or holds one-shot. Vertical sheet packing → each frame is a contiguous `gfx::Sprite` drawn by the existing `blit_scaled` (zero new renderer code); `--lab anim` demo plays a generated 8-frame spinner (`sprites/spin_8.hrt`) with fps/loop/restart | ✅ done |
...
| **Studio Sheet Export v1** | Track B — the Texture Lab exports **animated sheets**: `make_sheet` turns any tileable recipe into an N-frame seamless-scroll loop (`sprites/sheet_NN.hrt`); frame count self-describes via aspect ratio (`anim::frames_in_sheet` = h/w), so `--lab anim` and `--lab scene` auto-discover Studio output — the animation pipeline is now self-hosting (no script) | ✅ done |
...
| **Studio Shell v1** | Platform spine (Horizon 1) — the **shared Studio shell + dock/nav model** (`--shell`): a nav rail (Hub / Learn / About) over the domain, and an **interactive Hub** (`--shell`) where **Space publishes, 1/2 promote** in-window. The trigger is separated from the operation: `release_ops_core` (`publish`/`promote`/`rollback` → structured `OpResult`) is called by both the CLI and the Scenes, and is **end-to-end unit-tested** against a temp asset base (`test_release_ops`) — so the ops are verified and only the keypress→op wiring is manual. Both scenes are web-rendered too (`?mode=shell`). `docs/book/97` | ✅ done |
| **Hub Shell v1** | Platform spine (Horizon 1) — the **first Hub shell** as a view/controller: `--hub <path>` (headless) and the Studio's Hub section (**windowed**) both render one project's aggregate state (shippable? problems, source package hash, what each of `development`/`preview`/`production` points at + whether it `==source`) ending in the single **next recommended action** (fix → publish → promote→preview → promote→production → in-sync). Both renderers share one pure, unit-tested `hub_core::hub_lines`/`recommend` (CLI and window can't drift); `hub_build_core` assembles the view through `assets::`. One reference project, not a directory-scanning browser. Scene pixels are a manual visual accept. `docs/book/95`, `docs/book/96` | ✅ done |
...
cmake --build build
./build/demo --hub projects/creator.gameproject       # headless: one project's status + the next recommended action (Hub shell)
...
./build/demo --lab      # list the labs; then --lab <id>:
#   scene texture map editor    the Studio's Scene workspace full-screen / Texture Lab / Map Lab / UI+physics
#   fx light audio anim         particles / 2D lighting / mixer / sprite animation
#   3d viz3d iso colony         3D core / 3D sandbox / isometric farm sim / ECS+jobs+BaaS integration
