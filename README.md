...
| **Hub Shell v1** | Platform spine (Horizon 1) — the **first Hub shell** as a headless view/controller: `--hub <path>` aggregates one project's whole state (shippable? problems, source package hash, what each of `development`/`preview`/`production` points at + whether it `==source`) and ends in the single **next recommended action** (fix → publish → promote→preview → promote→production → in-sync). `hub_core::recommend` is a **pure, unit-tested** decision (the graphical Scene is deferred to the unblocked pixel path). One reference project, not a directory-scanning browser. `docs/book/95` | ✅ done |
