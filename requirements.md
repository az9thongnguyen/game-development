...
## 11. Định hướng mở rộng tương lai (optional — chưa làm ở M0–M5)

Ghi lại để định hướng kiến trúc, **không thực hiện ngay**:

- **Native webserver (ví dụ Drogon):** sau này có thể thêm một service C++ riêng
  (Drogon hoặc tương tự) để phục vụ bản WebAssembly, tính năng online/leaderboard,
  hoặc API. **Ràng buộc:** đây là một tiến trình / lớp **tách rời hoàn toàn**,
  KHÔNG kéo dependency hay logic của nó vào engine core. Engine vẫn web-portable
  qua Emscripten đúng như mục 8; webserver chỉ là tầng phân phối/dịch vụ bên ngoài,
  nối với game qua biên giới rõ ràng (HTTP/asset), không phải một phần của vòng đời
  engine.
- **Chess đủ chế độ (nhắc lại cho rõ):** chess phải có **cả Người-vs-Người và
  Người-vs-Máy** (AI minimax/alpha-beta tự viết, nhiều mức khó), và chạy được
  **cả GUI lẫn TUI** (mục 1 & 7). Không bỏ sót chế độ Người-vs-Máy.
