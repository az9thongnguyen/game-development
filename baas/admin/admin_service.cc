...
NewProject create_project(const std::string& name) {
    ...
    // secret stored hashed; plaintext returned once
    const long id = static_cast<long>(db::insert_id(db::client(),
        "INSERT INTO projects(name, public_key, secret_key_hash) VALUES(?,?,?)",
        name, pub, pw::hash(sec)));
...
std::vector<ProjectInfo> list_projects() {
    const auto rows =
        db::exec(db::client(), "SELECT id, name, public_key FROM projects ORDER BY id ASC");
