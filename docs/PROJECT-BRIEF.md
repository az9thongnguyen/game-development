...
| `docs/book/` (138 chapters) | The **explanation** of every subsystem, one chapter per feature | Before modifying a subsystem you did not write |
...
| Test suites | **88, all passing** (~39 s) — headless, no window. A build without Drogon registers **57**; the 31 Drogon-gated ones run in their own CI job since chapter 129, and every one of them now carries `TIMEOUT 120` (chapter 140: a self-deadlock cost 25 minutes of silence before ctest's default noticed). |
...
| Guidebook chapters | 141 (`docs/book/00`–`140`) |
...
docs/            book/ (138 chapters), strategy/ (6), guides/ (2), superpowers/{specs,plans}
...
| Postgres | ❌ **does not work** — measured 2026-09-07 (chapter 140), not inferred. `db.h` had said "a documented deploy-time build" since slice one; the first time anybody pointed the backend at a real Postgres, the first statement failed: `syntax error at or near ","  LINE 1: INSERT INTO schema_migrations(version, name) VALUES(?,?)`. Drogon does not translate `?` into `$1` and all **107** queries in `baas/` use `?`; underneath that, `id INTEGER PRIMARY KEY` is an auto-increment in SQLite and a plain column in Postgres, and the ten `insertId()` calls need `RETURNING`. `sh baas/ops/pg-test.sh` reproduces it in one command (Docker: `postgres:16-alpine` + the libpq-enabled `drogonframework/drogon` image). CI does not run it — a red job nobody can fix teaches nothing. Making it green is its own slice. |
