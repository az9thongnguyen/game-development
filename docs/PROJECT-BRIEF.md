...
| `docs/book/` (146 chapters) | The **explanation** of every subsystem, one chapter per feature | Before modifying a subsystem you did not write |
...
| Test suites | **94, all passing** (~39 s) — headless, no window. A build without Drogon registers **60**, measured; the 34 Drogon-gated ones run in their own CI job since chapter 129, and every one of them now carries `TIMEOUT 120` (chapter 140: a self-deadlock cost 25 minutes of silence before ctest's default noticed). |
...
| Guidebook chapters | 146 (`docs/book/00`–`145`) |
...
docs/            book/ (146 chapters), adr/ (the decision index), strategy/ (6), guides/ (2), superpowers/{specs,plans}
...
| **Architecture decision index** | ✅ added 2026-09-07 (chapter 143) — `docs/adr/README.md`, 58 decisions, each one line pointing at the chapter that argues it, and **11 marked `Superseded by N`** so a reversed decision is visible rather than folklore. No new prose: the chapter is the argument. `test_adr_index` checks the three ways such an index rots (a renamed chapter, a duplicated id, a supersession naming a row that is not there) and caught one on the first run. ⚠️ it checks LINKS, not CONTENT — a row that describes its decision wrongly still passes. |
