...
| **Architecture decision index** | ✅ added 2026-09-07 (chapter 143) — `docs/adr/README.md`, 49 decisions, each one line pointing at the chapter that argues it, and **8 marked `Superseded by N`** so a reversed decision is visible rather than folklore. No new prose: the chapter is the argument. `test_adr_index` checks the three ways such an index rots (a renamed chapter, a duplicated id, a supersession naming a row that is not there) and caught one on the first run. ⚠️ it checks LINKS, not CONTENT — a row that describes its decision wrongly still passes. |
