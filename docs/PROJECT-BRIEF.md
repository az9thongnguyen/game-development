...
it was pointed at.
**Last verified:** 2026-09-12, against the chapter-151 verification branch.
...
| `docs/book/` (152 chapters) | The **explanation** of every subsystem, one chapter per feature | Before modifying a subsystem you did not write |
...
| Test suites | **95, all passing** (36–63 s measured 2026-09-12) — headless, no window. A build without Drogon registers **60**, measured; the 35 Drogon-gated ones run in their own CI job since chapter 129, and every one of them now carries `TIMEOUT 120` (chapter 140: a self-deadlock cost 25 minutes of silence before ctest's default noticed). |
| SDL-free static libraries | 40, each with a matching `test_*` target |
...
| Guidebook chapters | 152 (`docs/book/00`–`151`) |
...
docs/            book/ (152 chapters), adr/ (the decision index), strategy/ (6), guides/ (2), superpowers/{specs,plans}
...
### ...and forty-four chapters after them (108–151), by theme

The table above stops at 107 because that is where the slices were still one-per-concern.
What followed was the PLAN-v2 correction roadmap (S19–S30c) plus six slices beyond it, and
it is more honestly read by theme than by row:

| Theme | Chapters | What changed |
|---|---|---|
| **Studio and UI** | 108–116, 120, 127, 132–133, 144, 147, 149–151 | UTF-8 + a real design system · `ui` v2 (clip, overlays, focus, layout) · workspaces as tabs *and* full-screen labs · one door per kind (12 flags → `--lab`) · a draggable split with a persisted layout · semantic controls and multi-touch · a product front door and readable game HUDs |
...
| **Games** | 113, 117, 124–126, 136–138, 143, 146, 151 | The farm (day loop, NPCs, cloud save, seasons as a RULE) · Creatures (18 species, integer battle, replay as a file, rated PvP a hand can play) · status and controls readable as product UI |
| **Web** | 118, 123, 128, 130, 150–151 | The page runs in CI, driven by a real finger · the collection a stranger can understand · the player retains product identity, a route back and viewport-safe controls |
...
| **Two-thumb input and semantic UI hierarchy** | ✅ verified 2026-09-08 (chapter 149): `InputState` carries ten independent, trivially-copyable contacts with stable ids and separate press/release edges; SDL maps them into logical framebuffer space and does not let its synthesized mouse duplicate them. A shared adapter feeds pure pointer spans to Farm and Creatures, where direction + verb work in one frame and order does not change intent. Farm's one layout now includes the HUD and hint neighbours a control must not cover. `ButtonKind`/`ButtonOptions`, vector icons, cards and meters give Studio and the release Hub semantic hierarchy instead of caller-picked colours; the navy/periwinkle token pass applies consistently across existing widgets. **21/21 mutations killed** after an overfilled meter survivor exposed a missing outside-the-rect assertion. ⚠️ no two-finger gesture has been driven on physical hardware or in a browser. ⚠️ `ui::Context` itself remains single-pointer; multi-touch currently serves game controls. |
| **A product front door and a named player** | ✅ verified 2026-09-12 (chapter 150): Collection now states what the engine is, gives each of five games a semantic card and dedicated cover, keeps Play at 50 px on a 390×844 viewport and never scrolls sideways. Creator and Farm gained hand-drawn `.pix` covers through the existing `asset.pixels` door; provenance reports 51 raster assets, zero unrecorded. The player reads the selected project's name from the manifest-baked `collection.json`, retains a route back, distinguishes loading from running and exposes both states of Log. A first full-suite run caught a subtle integration regression: declaring a `cover` again as editable `asset texture` added an inspector row and pushed Pixels' Save control out of 720p. The duplicate lines were deleted; covers remain in resource closure. **95/95 CTest passed twice**, Emscripten and the 390×844 Chrome journey passed, 20/21 mutations were killed with one equivalent target-size survivor, and Farm release `150f60129cb57b22` completed inspect → publish → verify → preview → Hub with zero `.tmp`. ⚠️ no physical phone or screen reader was used; fullscreen was not entered; the browser journey launches Colony while the other four links are inspected but not followed. |
| **Farm and Creatures read as games, not debug overlays** | ✅ verified 2026-09-12 (chapter 151): Farm presents calendar, numeric energy meter, gold and cloud state as four layout-owned cards; Creatures presents lead identity/health/progress, a safe message toast and two battle status cards. Touch controls name verbs (`USE`, `SEED`, `SAVE`, `PVP`, `ACT`) instead of keyboard glyphs. More than 800 viewport combinations keep status, controls, sprites and battle panel apart, with both sides of every new threshold covered. The web gate also found player-shell `min-height` was content-box: padding reduced the 390×844 stage to 771 px. `border-box` restored 785 px, and the browser now measures visibility, 44 px size and viewport containment for Back/Log/Fullscreen. **95/95 CTest passed twice**, 27/27 effective native mutations and 2/2 web mutations were killed, Emscripten plus Farm/Creatures/Collection journeys passed, and both game releases passed inspect/publish/parity with zero `.tmp`. ⚠️ no physical phone, screen reader or native window was used; a portrait 16:9 canvas still reduces 44 logical pixels to about 27 CSS pixels, and compact fallbacks below the new layout thresholds are not full product layouts. |
