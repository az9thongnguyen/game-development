...
it was pointed at.
**Last verified:** 2026-09-12, against the chapter-150 verification branch.
...
| `docs/book/` (151 chapters) | The **explanation** of every subsystem, one chapter per feature | Before modifying a subsystem you did not write |
...
| Guidebook chapters | 151 (`docs/book/00`–`150`) |
...
docs/            book/ (151 chapters), adr/ (the decision index), strategy/ (6), guides/ (2), superpowers/{specs,plans}
...
### ...and forty-three chapters after them (108–150), by theme

The table above stops at 107 because that is where the slices were still one-per-concern.
What followed was the PLAN-v2 correction roadmap (S19–S30c) plus five slices beyond it, and
it is more honestly read by theme than by row:

| Theme | Chapters | What changed |
|---|---|---|
| **Studio and UI** | 108–116, 120, 127, 132–133, 144, 147, 149–150 | UTF-8 + a real design system · `ui` v2 (clip, overlays, focus, layout) · workspaces as tabs *and* full-screen labs · one door per kind (12 flags → `--lab`) · a draggable split with a persisted layout · semantic controls and multi-touch · a product front door and named player |
...
| **Web** | 118, 123, 128, 130, 150 | The page runs in CI, driven by a real finger · the collection a stranger can understand · the player retains product identity and a route back |
