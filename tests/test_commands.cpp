...
static void test_asset_commands() {
    ...
    // Animation is authored in the same recipe and crosses the same door. A four-frame
    // 16px texture is stored as four square frames stacked vertically, so the runtime
    // can discover the frame count from the image shape without another sidecar.
    static const char kAnimatedRecipe[] =
        "seed=7\nsize=16\nbase=0\nbasis=0\nfrequency=4\noctaves=1\n"
        "gain=0.5\nlacunarity=2\nlo=4281031843\nhi=4284522206\n"
        "op=1\nop_amount=0.76\nframes=4\n";
    CHECK(assets::write_file("textures/_animated.recipe",
                             std::vector<std::uint8_t>(kAnimatedRecipe,
                                                       kAnimatedRecipe + std::strlen(kAnimatedRecipe))));
    const engine::OpResult animated =
        cmd::run("asset.texture", {"textures/_animated.recipe", "textures/_animated.hrt"});
    CHECK(animated.ok);
    const auto animated_bytes = assets::load_file("textures/_animated.hrt");
    CHECK(animated_bytes.has_value());
    if (animated_bytes) {
        const auto image = gfx::decode_hrt(*animated_bytes);
        CHECK(image.has_value());
        if (image) CHECK(image->w == 16 && image->h == 64);
    }
    std::filesystem::remove(ASSET_ROOT "/assets/textures/_animated.recipe");
    std::filesystem::remove(ASSET_ROOT "/assets/textures/_animated.hrt");
    ...
    // The 47-piece long-grass blob is held to the same source-of-truth rule. Its wide
    // sheet is especially easy to commit out of sync because most pieces are only
    // reached after the map's boundary changes.
    const auto committed_grass = assets::load_file("textures/creature_grass.hrt");
    CHECK(committed_grass.has_value());
    const engine::OpResult drew_grass =
        cmd::run("asset.pixels", {"textures/creature_grass.pix", "textures/_regrass.hrt"});
    CHECK(drew_grass.ok);
    const auto redrawn_grass = assets::load_file("textures/_regrass.hrt");
    CHECK(redrawn_grass.has_value());
    if (committed_grass && redrawn_grass) CHECK(*committed_grass == *redrawn_grass);
    std::filesystem::remove(ASSET_ROOT "/assets/textures/_regrass.hrt");
